...
class MOTModulatedTransformerCrossBlock(nn.Module):
    ...
    def _forward(self, x: torch.Tensor, mod: torch.Tensor, context: torch.Tensor):
        ...
        h = _tree_map(self._apply_module, x, self._moduledict_to_dict(self.norm1))
        h = _tree_map(
        ...
        h = _tree_map(self._apply_module, x, self._moduledict_to_dict(self.norm2))
        h = _tree_map(
        ...
        h = _tree_map(self._apply_module, x, self._moduledict_to_dict(self.norm3))
        h = _tree_map(
            partial(self._apply_mlp, scale_mlp=scale_mlp, shift_mlp=shift_mlp),
            h
        )
        h = _tree_map(self._apply_module, h, self._moduledict_to_dict(self.mlp))
        h = _tree_map(
