...
def render(
    ...
    if backend == "inria":
        ...
        # Convert all tensors to float32 for gsplat compatibility
        render_colors, render_alphas, meta = rasterization(
            means=means3D.float(),
            quats=rotations.float(),
            scales=scales.float(),
            opacities=opacity.squeeze(-1).float(),
            colors=gsplat_colors.float() if gsplat_colors is not None else None,
            sh_degree=gsplat_sh_degree,
            viewmats=viewpoint_camera.world_view_transform.T.contiguous().unsqueeze(0).float(),
            Ks=Ks.float(),
