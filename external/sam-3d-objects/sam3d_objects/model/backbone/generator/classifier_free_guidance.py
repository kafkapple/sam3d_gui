...
from sam3d_objects.model.backbone.tdfy_dit.modules.pytree_compat import tree_map_compat as _tree_map
...
def _zeros_like(struct):
    ...
    return _tree_map(make_zeros, struct)
...
class ClassifierFreeGuidance(torch.nn.Module):
    ...
    def _cfg_step(self, y_cond, y_uncond, strength):
        if isinstance(strength, dict):
            return _tree_map(self._cfg_step_tensor, y_cond, y_uncond, strength)
        else:
            return _tree_map(partial(self._cfg_step_tensor, strength=strength), y_cond, y_uncond)
...
def get_strength(strength, interval, t):
    if interval is None:
        return _tree_map(lambda x: 0.0, strength)
...
class PointmapCFG(ClassifierFreeGuidance):
    ...
    def _cfg_step(self, y_cond, y_uncond, y_pm, strength, strength_pm):
        if isinstance(strength, dict):
            return _tree_map(self._cfg_step_tensor, y_cond, y_uncond, y_pm, strength, strength_pm)
        else:
            return _tree_map(partial(self._cfg_step_tensor, strength=strength, strength_pm=strength_pm), y_cond, y_uncond, y_pm)
